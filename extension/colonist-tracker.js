...
export function formatResourceEnumList(arr) {
  ...
  return arr
    .map((id) => RESOURCE_ID[Number(id)] ?? `#${id}`)
    .join(", ");
}
...
export function inferSeatCountFromTracker(state) {
  const cs = state?.currentState && typeof state.currentState === "object" ? state.currentState : {};
  for (const k of ["maxPlayers", "playerCount", "numberOfPlayers", "seatCount", "totalPlayers"]) {
    const v = toFiniteNumber(cs[k]);
    if (v != null && v >= 2 && v <= 8) return v;
  }
  const p = state.players && typeof state.players === "object" ? state.players : {};
  let active = 0;
  for (const id of Object.keys(p)) {
    ...
    if (pl.bankRoadAmount != null || pl.bankSettlementAmount != null || pl.bankCityAmount != null) {
      active++;
      continue;
    }
    ...
    if (wr && typeof wr === "object") {
      let t = 0;
      for (const rn of ["lumber", "brick", "wool", "grain", "ore"]) t += Number(wr[rn]) || 0;
      if (t > 0) {
        active++;
        continue;
      }
    }
    if (Array.isArray(pl.developmentCardsUsed) && pl.developmentCardsUsed.length > 0) {
      active++;
    }
  }
...
function mergeBankResourceCards(state, rc) {
  ...
  if (!state.bank || typeof state.bank !== "object") state.bank = emptyResources();
  if (!state.bank.other || typeof state.bank.other !== "object") state.bank.other = {};
...
function applyPlayerStatesPatch(state, playerStates) {
  ...
  for (const colorKey of Object.keys(playerStates)) {
    ...
    if (!state.players[pid]) state.players[pid] = { colorId: Number(colorKey) };
    if (p.resourceCards) {
      state.players[pid].wireResources = normalizeResourceCards(p.resourceCards);
      const rawCards = p.resourceCards.cards;
      if (Array.isArray(rawCards) && rawCards.length > 0) {
        state.players[pid].wireHandSlotCount = rawCards.length;
        state.players[pid].wireHandAllZeros = rawCards.every((x) => Number(x) === 0);
      } else {
...
function noteUnknownDiffKeys(diffRoot, state) {
  if (!diffRoot || typeof diffRoot !== "object" || Array.isArray(diffRoot)) return;
...
function extractTimeLeftInState(parsed) {
  try {
    ...
    if (p && typeof p === "object" && typeof p.timeLeftInState === "number") return p.timeLeftInState;
  } catch {
    /* ignore */
  }
  return null;
...
function applyTradeStatePatch(state, ts) {
  ...
  if (ts.activeOffers && typeof ts.activeOffers === "object") {
    if (!m.activeOffers || typeof m.activeOffers !== "object") m.activeOffers = {};
  ...
  if (ts.closedOffers && typeof ts.closedOffers === "object") {
    if (!m.closedOffers || typeof m.closedOffers !== "object") m.closedOffers = {};
...
function visitForPatches(obj, acc) {
  ...
  if (obj.payload != null && typeof obj.payload === "object") visitForPatches(obj.payload, acc);
  if (obj.data != null && typeof obj.data === "object") visitForPatches(obj.data, acc);
  if (obj.diff != null && typeof obj.diff === "object") visitForPatches(obj.diff, acc);
  ...
  if (obj.mechanicSettlementState) acc.settlementPatches.push(obj.mechanicSettlementState);
  ...
  if (obj.mechanicLongestRoadState) acc.longestRoadPatches.push(obj.mechanicLongestRoadState);
  if (obj.mechanicLargestArmyState) acc.largestArmyPatches.push(obj.mechanicLargestArmyState);
  ...
  if (acc.seenDiffRoots && isProbablyDiffPatch(obj) && !acc.seenDiffRoots.has(obj)) {
...
export function applyParsedMessage(state, parsed) {
  ...
  if (!state.currentState || typeof state.currentState !== "object") state.currentState = {};
  for (const patch of acc.currentStatePatches) {
    if (patch && typeof patch === "object") Object.assign(state.currentState, patch);
  }
  ...
  for (const sp of acc.settlementPatches) {
    applyPieceMechanic(state, "bankSettlementAmount", sp, "bankSettlementAmount");
  }
...
export function applyGameLogDelta(state, detail) {
  if (!detail || typeof detail !== "object" || typeof detail.cards !== "object") return state;
...
export function feedHandRowForColorId(state, colorId) {
  ...
  const byHex = state.logHandByColorHex && typeof state.logHandByColorHex === "object" ? state.logHandByColorHex : {};
  const preferred = normalizeColonistChatHex(COLONIST_CHAT_HEX_BY_COLOR_ID[cid]);
...
export function applyAnalystPayload(state, payload) {
  if (payload?.kind !== "ws-message" && payload?.kind !== "ws-send") return state;
...
export function buildHeuristics(state) {
  ...
  if (scarcest < Infinity && scarcest >= 2 && scarcestNames.length > 0) {
    tips.push(`Scarcest in bank right now: ${scarcestNames.join(", ")} (${scarcest}).`);
  }
  ...
  const warnAt = twoPlayer ? ROBBER_DISCARD_HAND_LIMIT_TWO_PLAYER : standardLimit + 1;
  ...
  for (const pid of Object.keys(players)) {
    ...
    if (hidden && cardCount >= warnAt) {
      tips.push(
        `${playerLabel(pl.colorId ?? Number(pid))} has ${cardCount} hidden cards on the wire (robber discard if ${discardPhrase}).`,
      );
    } else if (!hidden && total >= warnAt) {
      tips.push(
        `${playerLabel(pl.colorId ?? Number(pid))} holds ${total} cards (robber discard if ${discardPhrase}).`,
      );
    }
  }
  return tips;
